...
namespace NewRelic.Microsoft.SqlServer.Plugin.Configuration
{
    public class Settings
    {
        ...
        {
            return string.Format("Version: {0}, UseSsl: {1}, PollIntervalSeconds: {2}, SqlServers: {3}, CollectOnly: {4}",
                                 Version,
                                 UseSsl,
                                 PollIntervalSeconds,
                                 string.Join(",", SqlServers.Select(s => string.Format("[{0}]", s.ToString())).ToArray()),
                                 CollectOnly
                );
        }
    }
