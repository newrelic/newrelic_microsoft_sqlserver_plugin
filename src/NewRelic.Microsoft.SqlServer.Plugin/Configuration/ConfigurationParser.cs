...
namespace NewRelic.Microsoft.SqlServer.Plugin.Configuration
{
	internal static class ConfigurationParser
	{
		...
		///     Optional. If null, then uses the loaded app.config. Otherwise attempts to find and load the <paramref name="configFilePath" />.
		/// </param>
		...
		/// <param name="log">Log4Net ILog implementation</param>
		/// <returns>Settings parsed from <paramref name="configFilePath" />.
		/// </returns>
		private static Settings LoadConfigurationFromFile(string configFilePath, ILog log)
		{
			...
			var settingsFromConfig = Settings.FromConfigurationSection(section);
		...
		private static Settings GetSettingsFromAppConfig(ILog log)
		{
			...
			var settingsFromAppConfig = Settings.FromConfigurationSection(section);
			log.DebugFormat("Settings loaded successfully");
