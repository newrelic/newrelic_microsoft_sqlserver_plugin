...
namespace NewRelic.Microsoft.SqlServer.Plugin.Core
{
	internal class PollingThread
	{
		...
		private void ThreadLoop()
		{
			...
			if (ThreadSettings.InitialPollDelaySeconds > 0)
			{
				_log.DebugFormat("{0}: Initial Delay Sleep for {1}", ThreadSettings.Name, ThreadSettings.InitialPollDelaySeconds);
				ThreadSettings.AutoResetEvent.WaitOne(TimeSpan.FromSeconds(ThreadSettings.InitialPollDelaySeconds), true);
			}
