...
namespace NewRelic.Microsoft.SqlServer.Plugin.Core
{
	...
	public sealed class QueryAttribute : Attribute
	{
		...
		/// <param name="metricPattern"><see cref="MetricPattern"/></param>
		...
		/// <para><em>Custom/FileIO/{DatabaseName}/{MetricName}</em></para>
		/// <para>The pattern placeholders are replaced when recording the metric. Support includes:</para>
		/// <para>{DatabaseName} - <em>Replaced by <see cref="IDatabaseMetric.DatabaseName"/> when the <see cref="Type.ReflectedType"/> implements <see cref="IDatabaseMetric"/></em></para>
		/// <para>{MetricName} - <em>Replaced by <see cref="MetricAttribute.MetricName"/> when applied to a given property. If not applied, the convention is to use the value of <see cref="PropertyInfo.Name"/></em></para>
		/// </summary>
