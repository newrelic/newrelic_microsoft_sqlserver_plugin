...
SELECT
    ...
    CAST ([TWS1].[Percentage] AS DECIMAL(4, 2)) AS [Percentage],
    CASE
		WHEN 
			ISNULL([TWS1].[WaitCount],0) = 0 THEN 0
			ELSE CAST (([TWS1].[WaitSeconds] / [TWS1].[WaitCount]) AS DECIMAL (14, 4))
	END AS [AvgWaitSeconds],
    CASE
		WHEN 
			ISNULL([TWS1].[WaitCount],0) = 0 THEN 0
			ELSE CAST (([TWS1].[ResourceSeconds] / [TWS1].[WaitCount]) AS DECIMAL (14, 4)) 
	END AS [AvgResourceSeconds],
    CASE
		WHEN 
			ISNULL([TWS1].[WaitCount],0) = 0 THEN 0
			ELSE CAST (([TWS1].[SignalSeconds] / [TWS1].[WaitCount]) AS DECIMAL (14, 4)) 
	END AS [AvgSignalSeconds]
FROM [TempWaitStates] AS [TWS1]
	JOIN [TempWaitStates] AS [TWS2]
    ON [TWS2].[RowNum] <= [TWS1].[RowNum]
GROUP BY [TWS1].[RowNum], [TWS1].[wait_type], [TWS1].[WaitSeconds],
    [TWS1].[ResourceSeconds], [TWS1].[SignalSeconds], [TWS1].[WaitCount], [TWS1].[Percentage]
HAVING SUM ([TWS2].[Percentage]) < 95 -- percentage of whole threshold
